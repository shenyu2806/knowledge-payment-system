    ...
    <el-table v-loading="loading" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="identity" label="用户类型" align="center">
        <template #default="{row}">
          <el-tag type="success" v-if="row.identity == 1">讲师</el-tag>
...
import userinfo from '../components/teacherinfo.vue'
...
//获取表格数据
const getList = async () =>{
  const res = await getAdmingListLength("1",pagination.pagesize,pagination.id)
  if(res.status == 0){
...
const gettotal= async (num) =>{
  if(num == "n"){
    ...
    pagination.total = res.data[2].num
  }else{
...
const serte = async () =>{
  ...
  loading.value = true
  const res = await getserteList("1",seactVe.value)
  gettotal(res.length)
  tableData.value = res.result
  await new Promise(resolve => setTimeout(resolve, 1000));//等待1秒
  loading.value = false
